...
def visualize_embeds_3d(predictions: pandas.DataFrame):
    ...
    for model in ["human", "base", "adapter"]:
        predictions[(model, "text_embeds")] = (
            transformer(
                **tokenizer(
                    predictions[("text", model)].tolist(),
                    padding=True,
                    return_tensors="pt",
                )
            )
        ...
        predictions[(model, "text_embeds_d3")] = list(
            PCA(n_components=3).fit_transform(
                predictions[(model, "text_embeds")].tolist()
            )
        )
    ...
    plotly.offline.iplot(
        go.Figure(
            ...
            layout=go.Layout(margin={"l": 0, "r": 0, "b": 0, "t": 0}, height=820),
        )
    )
