...
def filter_tweets(
    ...
    return (
        ...
        .pipe(
            lambda _df: _df[~_df["text"].str.startswith("@")]
            if remove_w_mentions
            else _df
        )
        # exclude retweets
        .pipe(
            lambda _df: _df[~_df["text"].str.contains("RT")] if remove_retweets else _df
        )
        ...
        # remove line breaks
        .pipe(
            lambda _df: _df.assign(text=_df["text"].str.replace(r"\n", " ", regex=True))
        )
    )
...
def format_reply_instructions_dataset(
    ...
    return [
        ...
        for sample in itertools.combinations(
            list(group[["text_post", "text_reply"]].itertuples(index=False)),
            n_shots + 1,
        )
